...
`Jetton Transfer`([TEP-74](https://github.com/ton-blockchain/TEPs/blob/master/text/0074-jettons-standard.md#1-transfer))의 `body`는 일반적으로 다음과 같은 방식으로 작성되어야 합니다:
...
`Jetton Transfer`([TEP-74](https://github.com/ton-blockchain/TEPs/blob/master/text/0074-jettons-standard.md#1-transfer))에서 `messageBody`에 댓글을 추가하려면, 일반적인 전송 `body`에 더해 댓글을 직렬화하고 이를 `forwardPayload`에 포함시켜야 합니다.
...
`Jetton Burn`([TEP-74](https://github.com/ton-blockchain/TEPs/blob/master/text/0074-jettons-standard.md#2-burn))의 `body`는 일반적으로 다음과 같은 방식으로 작성되어야 합니다:
...
여기 GetGems 마켓플레이스에서 [nft-fixprice-sale-v3r2](https://github.com/getgems-io/nft-contracts/blob/main/packages/contracts/sources/nft-fixprice-sale-v3r2.fc) 계약에 따라 메시지와 거래를 준비하는 예시가 있습니다.
...
[nft-fixprice-sale-v3r2](https://github.com/getgems-io/nft-contracts/blob/main/packages/contracts/sources/nft-fixprice-sale-v3r2.fc) 판매 계약에서 NFT를 구매하는 과정은 페이로드가 없는 일반적인 전송으로 수행할 수 있으며, 중요한 것은 정확한 TON 금액입니다. 그 금액은 다음과 같이 계산됩니다:\
